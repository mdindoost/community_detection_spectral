...
\begin{itemize}
    ...
    \item \textbf{A boundary characterization}: genuine runtime-matched gains exist on at most 2 of 15 networks (one robust, one single-configuration candidate), and no tested structural statistic predicts them: an existence proof, an open problem, and a demonstration that predictor claims in this regime require far larger samples.
...
must place surplus edges externally: a node of degree $d$ in a community
of $n_c$ nodes has at least $d - n_c + 1$ external edges whenever $d \ge n_c$.
...
When the condition holds, degree-based sparsification tends
...
\end{enumerate}
Whether hub-bridging holds depends on the partition used and, as Section~\ref{sec:mech-null} shows, is not by itself informative about community structure: three networks in our suite violate it under Leiden partitions while retaining $\delta > 0$, and every rewired null satisfies it.
...
\begin{proof}
From Theorem~\ref{thm:ratio}(a), the ratio is $\mu_{\text{inter}}/\mu_{\text{intra}}$. Substituting $\mu_{\text{inter}} = \mu_{\text{intra}} - \delta$:
\begin{equation}
...
\textbf{(B) Variance Form:} Equivalently, since the total degree is fixed under both weightings, the sum of squares does not increase if and only if the variance of community degree sums does not increase:
\begin{equation}
...
\textbf{(B)} For any set of values $\{x_c\}$ with fixed sum $S = \sum_c x_c$, we have $\sum_c x_c^2 = k \cdot \text{Var}_c(x_c) + S^2/k$. Since $\sum_c D_c^{(s)} = \sum_c D_c^{(1)} = 2m$, variance reduction and sum-of-squares reduction are equivalent.
...
Tables~\ref{tab:exp1_2_modularity}--\ref{tab:exp1_2_decomposition} retain the full graphs as downloaded, matching the pipeline that produced them; every subsequent experiment in this paper uses the largest connected component of each simple undirected graph, so edge counts differ slightly between these tables and later ones. The Artifact~I sweep (Section~\ref{sec:artifact-modularity}) and the boundary study (Section~\ref{sec:boundary}) likewise use overlapping but not identical 15-network subsets of the 17-network suite; each table lists its own membership.
...
Because these runs use the with-replacement sampler, the Bernoulli-model ratio of Corollary~1 is not expected to hold exactly, and it does not: measured preservation ratios are systematically closer to one than predicted, by $0.07$ to $0.30$ across datasets, which is the quantitative signature of that sampler's concave retention profile (Section~\ref{sec:sampler}). The direction of preferential removal agrees with the prediction wherever $\delta$ is materially positive.
...
Presented without controls, such measurements read as evidence of improvement; the controls below explain why that reading fails.
...
Positive $\delta$, positive $\Delta Q_{\mathrm{fixed}}$, and hub-bridging above one are properties of heavy-tailed degree sequences interacting with modularity optimization; their sign carries no information about community structure.
...
In our measurements the pipeline costs $1.55$--$1.90\times$ a single Leiden run (sparsification itself is only $4$--$8\%$ of the total), so the matched baseline is best-of-two restarts. Best-of-two costs $2.0\times$ a single run, slightly more than the pipeline's $1.55$--$1.90\times$; the slack favors the baseline, which is conservative for reported gains and mildly anti-conservative for reported losses.
...
The seeded pipeline beats the runtime-matched baseline beyond twice the baseline's seed deviation on exactly \textbf{2 of 15} networks: \texttt{email-Enron} ($+0.0077$, $4.1$ baseline standard deviations) and \texttt{com-Youtube} ($+0.0086$, $2.5$).
The \texttt{com-Youtube} entry warrants caution: it rests on a single configuration, and its matched baseline is a single best-of-two realization that landed one standard deviation below the baseline mean; measured against the baseline mean the gain is $+0.0051$, and against the expected best-of-two roughly $+0.003$, under the two-deviation bar. No multiplicity correction is applied across the 15 networks; at this threshold roughly half a false positive is expected. We therefore report one robust gain and one candidate.
The \texttt{email-Enron} result is robust in every direction we probed: it holds for both sampler variants tested there (calibrated and clipped no-replacement), at every retention level ($0.7$--$0.95$), against best-of-five as well as best-of-two baselines, and it strengthens under \emph{harsher} sparsification (up to $+0.0149$ under the clipped no-replacement sampler at retention $0.7$). The gain therefore stems from aggressive hub-edge removal changing the optimizer's search space, not from mild denoising.
