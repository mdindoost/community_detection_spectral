...
All experiments in this paper use the largest connected component of each simple undirected graph. The Artifact~I sweep (Section~\ref{sec:artifact-modularity}) and the boundary study (Section~\ref{sec:boundary}) likewise use overlapping but not identical 15-network subsets of the 17-network suite; each table lists its own membership.
...
Cross-graph comparison thus reverses the sign of the conclusion on every dataset: what appears as a gain of $+0.01$ to $+0.30$ is, on the fixed objective, a loss of $0.004$ to $0.19$.
