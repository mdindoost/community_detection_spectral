...
A stronger claim appears in several places.
Satuluri et al.~\cite{satuluri2011local} report that similarity-based sparsification improves scalable clustering; backbone extraction is motivated as revealing structure obscured by weak edges~\cite{serrano2009extracting, coscia2017backboning}; DSpar~\cite{liu2023dspar} establishes that, with importance reweighting and under conditions on the graph, degree-based sampling approximates spectral sparsification, and reports preserved downstream accuracy at scale; and an earlier version of this work reported consistent modularity improvements from DSpar preprocessing across a broad collection of real-world networks (unpublished manuscript).
...
With importance reweighting of the retained edges, and under conditions relating degrees to effective resistances, DSpar approximates effective-resistance sparsification and thus inherits approximate spectral guarantees~\cite{liu2023dspar}; this guarantee is attached to the \emph{weighted} sparsified graph, a distinction that will matter throughout.
...
We apply the with-replacement DSpar sampler at nominal $\alpha = 0.8$ (realized retention $0.33$--$0.48$ on these eleven networks; Section~\ref{sec:sampler}), 10 independent runs, and evaluate the fixed partition on the sparsified graph; retained edges are kept unweighted, matching common practice in community-detection pipelines and the audited experiments.
...
Nor do these findings contradict DSpar's spectral guarantee: a $(1 \pm \epsilon)$ sparsifier preserves the cut weights and weighted degrees that determine fixed-partition modularity to $O(\epsilon)$, but the guarantee requires the importance weights, and the pipelines audited here, as in common practice for community detection, discard them. The artifacts we document are properties of \emph{unweighted} degree-biased deletion, precisely the regime the guarantee does not cover.
...
The standard evidence that this mechanism improves detection is artifactual: it is attributable to sparse-graph self-scoring, granularity, and unmatched computation, and it is reproduced in full by configuration models without community structure.
