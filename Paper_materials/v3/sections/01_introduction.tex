...
% [B3: the line of work that followed, with citations]
...
reference implementations, which are now distributed in standard graph
libraries. Wu and Chen~\cite{wu2020gsgan} trained a generative model to sparsify
graphs specifically for community detection, reporting that clustering algorithms
retain accuracy while ``only using far fewer edges (e.g., 5\%).''
Sparsification also entered production: the SimClusters system at
...
report the converse effect, that sampling can promote apparent community
structure in social and information networks. Most recently, Chen et
...
Across this line of work the recommendation broadened. Sparsify-then-detect has
been applied to modularity optimizers as well as to fixed-$k$ cut partitioners,
to sparse graphs as well as to dense ones, and evaluated against graph-internal
objectives as well as against external references. The degree threshold reported
in the original synthetic sweep does not reappear, and the conditions attached to
the original result are not generally restated alongside it.
