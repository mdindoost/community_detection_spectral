...
further requirements of the same kind. A sparsifier that removes edges
preferentially between communities interacts with quality measures that penalize
exactly those edges. A sparsifier that selects edges by degree reproduces effects
that a heavy-tailed degree sequence produces on its own, independently of any
community structure. A detection algorithm free to choose its own granularity
returns finer partitions on a sparsified graph, and the standard recovery scores
increase with the number of clusters. A pipeline that adds a preprocessing step
must be charged for it when its running time is compared with anything else. Each
of these is a property of a specific method class rather than a general caveat,
and each determines what a comparison involving that class is entitled to
conclude. Section~\ref{sec:protocol} states the resulting requirements and the
control that establishes each one.
