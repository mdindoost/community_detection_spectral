...
algorithm that partitions it, whether by modularity optimization, flow
simulation, label propagation or spectral bisection, pays for those links on
every pass. The nodes cannot be discarded, because a community assignment is
required for each of them; sampling vertices changes the problem rather than
shrinking it. Edges are different. If most of them are redundant for the purpose
of recovering community structure, then removing them before detection would buy
speed for nothing. That possibility has motivated fifteen years of work on
sparsify-then-detect pipelines, and the methods that implement it now ship in
widely used graph libraries and in at least one production system.
...
are measured against clustering runs that take hours. The graphs on which the
gains are largest are dense, with average degrees of 65, 76 and 94, and two of
them are known to contain a substantial fraction of spurious edges. The authors
also report, in a synthetic sweep, that their method ``actually outperforms the
original clustering starting from degree 50.'' That sentence names a boundary,
and as far as we can determine no subsequent work returns to it.
...
extensions is unreasonable on its face, and each was made in good faith. Together
they moved the claim off the territory where it was established, and the degree
threshold that would have marked the move was not carried along.

% [B4: the lost control and what its absence costs (THE HINGE)]
One methodological requirement was lost with it, and its loss is why the boundary
has stayed invisible. A sparsifier for community detection is built to remove
edges lying between communities in preference to edges lying inside them. A graph
so treated is a graph on which any partition looks better, because the very edges
that penalize a partition have been preferentially deleted. Measuring the quality
of a partition on the sparsified graph does not measure how well sparsification
preserved community structure. It measures how thoroughly the sparsifier did its
own job. The correct comparison scores the partition obtained from the sparsified
graph and the partition obtained from the original graph on the \emph{same}
graph, the original one, so that a single yardstick is applied to both.
...
Its absence is not a minor bias. Across 63 controlled configurations spanning
four detection algorithms, two sparsifier families and seven networks, scoring on
the sparsified graph rather than the original reverses the sign of the conclusion
in 45 of them. A pipeline that is harmful by 0.40 modularity under the correct
...
granularity, on the sparse graphs that dominate practice, with average degrees
between 5 and 33, no sparsifier we tested improves modularity measured on the
original graph beyond the variation of a few random restarts, at any retention
level that preserves quality, under any of four algorithms. There is no speed
argument either. End to end, including the sparsifier's own cost, the pipeline
runs at between $0.87\times$ and $1.35\times$ the time of clustering the original
graph directly, and at aggressive retention it can be slower, because a
fragmented graph gives the optimizer more work rather than less. Every apparent
...
Two facts travel with the positive result. Sparsification repairs a weaker
