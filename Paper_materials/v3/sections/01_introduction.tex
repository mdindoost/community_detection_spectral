...
Evaluating the first partition on the sparsified graph instead measures something
else. A sparsifier intended for community detection removes edges lying between
communities in preference to edges lying within them, and quality measures for a
partition, including modularity and conductance, penalize exactly those
between-community edges. A partition scored on the sparsified graph is thus
scored against a graph from which part of its own penalty term has been deleted,
and the resulting value reflects how selectively the sparsifier removed edges
rather than how much community structure survived. The two quantities are not
comparable, and only the first answers the question.
