...
Of the 63 configurations in Table~\ref{tab:accounting}, four preserve quality, in
the sense of coming within $0.005$ of the best-of-$N$ baseline, and the greatest
end-to-end speedup among them is $0.66\times$, which is slower than not
sparsifying. Speedups above $1.5\times$ occur in 12 of the 63 cells, and all 12
lose modularity, by $0.015$ to $0.162$ against the compute-matched baseline.
Discounting the sparsifier entirely and timing only the detection step, the
median speedup is $1.21\times$ for Infomap, $1.63\times$ for Louvain and
$1.15\times$ for label propagation, well short of the factor of two that halving
the edges suggests, because all three algorithms return more communities on a
sparsified graph, on average $1.4$, $8.9$ and $2.3$ times as many, and do more
work per pass.
