...
Every number reported in the rest of this paper is produced under the controls
stated here; Table~\ref{tab:protocol} collects them.

\subsection{One yardstick for two partitions}\label{sec:protocol:transfer}

A sparsify-then-detect pipeline returns a partition of the vertex set, and so
does the detection algorithm applied to the original graph. Both describe the
same vertices, and the question is which of them describes $G$ better, so both
are scored on $G$:
\begin{equation}\label{eq:honest}
\Delta Q \;=\; Q(P_{\mathrm{sparse}}, G) \;-\; Q(P_{\mathrm{base}}, G),
\qquad P_{\mathrm{sparse}} = \mathcal{A}(G'),
\end{equation}
where $\mathcal{A}$ is the detection algorithm and $G'$ the sparsified graph.
Detection may use any graph; scoring uses the original. The alternative
accounting scores $P_{\mathrm{sparse}}$ on $G'$ and $P_{\mathrm{base}}$ on $G$,
and so compares two numbers computed on two different objects.

The requirement is stated in the paper that introduced the pipeline. Measuring
quality on the sparsified graph, Satuluri et al.\ write, ``would tell us nothing
about how well the sparsified graph retained the cluster structure in the
original graph''~\cite[\S4.3]{satuluri2011local}, and their reported conductances
are computed on the original graph throughout.

Why the two accountings differ is visible in the objective. Modularity scores a
partition $P$ of a graph with $m$ edges by
...
The inflation does not require a sparsifier that selects intelligently. Uniform
random edge deletion, whose selection rule uses no information at all, was scored
both ways on three networks at two retention levels. Modularity measured on the sparsified graph rises in all six cells, by up
to $+0.011$. The same partitions scored on the original graph fall in all six, by
up to $-0.025$. The sign of the conclusion is opposite in every cell, and the gap
between the two accountings roughly doubles when retention falls from $0.9$ to
$0.8$.

Across a larger grid the pattern holds under every algorithm we tested. In 63
configurations spanning three detection algorithms, seven networks and three
sparsifier settings, modularity read off the sparsified graph is positive in 60
of them and reaches $+0.537$. The same partitions scored on the original graph
are positive in 15, and 11 of those survive a compute-matched baseline. Between
the two accountings the sign of the conclusion differs in 45 of the 63
configurations, and separately by algorithm in 14 of 21 under Infomap, 20 of 21
under Louvain, and 11 of 21 under label propagation.
...
magnitude across a pruning sweep. Best-match $F_1$, purity and normalized mutual
information all increase with the number of communities, so a recovery comparison
between partitions of different granularity measures that difference along with
whatever else it is meant to measure. Hamann et al.~\cite{hamann2016structure}
published the warning for normalized mutual information and recommended
chance-corrected indices in its place; the control below operationalizes it.
...
\eqref{eq:modularity} until the counts agree. On com-DBLP, Louvain returns 220
communities on the original graph and scores $0.102$ average best-match $F_1$
against the reference communities. Local similarity sparsification at retention
$0.5$ returns 10,947 communities and scores $0.193$, an apparent gain of $+0.091$.
The original graph partitioned at $\gamma = 576$ returns 11,159 communities and
scores $0.300$. The finer partition is worth twice what the sparsification
appeared to be worth, and it requires no sparsifier. Leiden reproduces both
numbers to three decimal places, which places the effect in the modularity
objective's default granularity rather than in one optimizer's search.
...
treatment. A result counts as surviving only if it survives both. The controls
compose, so the granularity control is applied to results that have already
passed the compute test.
...
Score both partitions on the original graph, Eq.~\eqref{eq:honest} &
