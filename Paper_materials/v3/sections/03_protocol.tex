...
measure when it is absent, so that a reader can judge the size of the correction
rather than take the requirement on principle. Table~\ref{tab:protocol} collects
them. Every number in the sections that follow is produced under all of them.
...
intelligently. Uniform random edge deletion, whose selection rule uses no
information at all, was scored both ways on three networks at two retention
levels. Modularity measured on the sparsified graph rises in all six cells, by up
