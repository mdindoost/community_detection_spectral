    ...
    if do_metis:
        ...
        for target in ops:
            for arm in ARMS:
                ...
                fsl = []
                gs = None
                for i, s in enumerate(METIS_SEEDS):
...
if __name__ == "__main__":
    if len(sys.argv) < 3 or sys.argv[1] != "net":
        raise SystemExit("usage: run.py net <network> [--metis] [--metis-only] [--k-mode nc_base|gt]")
    METIS_ONLY = "--metis-only" in sys.argv
    km = sys.argv[sys.argv.index("--k-mode") + 1] if "--k-mode" in sys.argv else "nc_base"
    run_network(sys.argv[2], do_metis=("--metis" in sys.argv or METIS_ONLY), k_mode=km)
