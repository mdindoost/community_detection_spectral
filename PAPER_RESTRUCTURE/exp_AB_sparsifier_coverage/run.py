...
def run_network(name, do_metis=False):
    import networkit as nk
    ...
    if mode == "hard":
        ...
        log(f"  labels: {int((y >= 0).sum())}/{n} nodes, "
            f"{len(set(y[y >= 0].tolist()))} classes")
    elif mode == "overlap":
...
if __name__ == "__main__":
    if len(sys.argv) < 3 or sys.argv[1] != "net":
        raise SystemExit("usage: run.py net <network> [--metis]")
    run_network(sys.argv[2], do_metis="--metis" in sys.argv)
