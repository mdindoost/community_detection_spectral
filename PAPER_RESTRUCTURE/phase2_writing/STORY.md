...
| C8 | Real structure suppresses δ vs null (16/17, median ratio 1.60). Mechanism localized to partition–score misalignment, not granularity — now on ALL 17/17 networks (sorting term positive 16/16, median +0.57; class-balance-free AUC cross-check tracks the ordering, Spearman +0.84); triangle/clustering DEAD at full coverage (tri_intra +0.741→+0.187, C +0.533→+0.126 — the jackknife's forecast vindicated); heavy tail = score-blind tail (com-Amazon auc_s 0.475, cit-Patents 0.499 ≈ chance); hub_inter_lift the only non-circular survivor (Spearman -0.635, uncorrelated with r_real at +0.06) → Exp Q's target. Root cause still open | exp_B; exp_M SUMMARY (17/17) | Done, in paper; C8 wording update pending tex |
