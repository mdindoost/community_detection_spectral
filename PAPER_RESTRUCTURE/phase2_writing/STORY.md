...
| C16 | L-Spar (Satuluri 2011, the founding sparsify-then-detect method): quality claim fails 14/14 cells under honest scoring (dQ_vs_matched -0.014..-0.404; naive scoring flips the sign in every cell = Artifact I); BUT its Jaccard signal is genuinely structure-aware — the only sparsifier whose selection signal the config null does NOT reproduce (deltaJ collapses to ~0/negative on rewirings). dQ_fixed retired as evidence (null reproduces it fully). Recovery: granularity control kills 5/6 cells; com-Amazon@0.542 exception (+0.062 vs over-matched control) — PROMOTED to result by exp_V's pre-registered adjudication (survives +37%-k over-matched control AND size-matched chance floor; coexists with modularity loss -0.079: objective/recovery dissociation). No speedup (0.87-1.35x) | exp_L SUMMARY; exp_V V5 | Verified; NOT yet in paper — new section needed |
