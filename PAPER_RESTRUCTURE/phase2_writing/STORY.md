...
| C8 | Real structure suppresses δ vs null (16/17). Mechanism localized to partition–score misalignment, not partition granularity (exp_M, 11/17 networks; exact r·sd/√(p(1-p)) decomposition, sorting term positive 10/10); triangle/clustering explanation unsupported (correlation collapses under jackknife; staged probe saturated). Root cause still open | exp_B; exp_M SUMMARY | Done, in paper; C8 wording update pending tex |
| C9 | Genuine gains: Enron robust (+0.008, 8/8 configs); Youtube small, regime-confined (+0.003). Enron anatomy (exp_N): survives adversarial granularity check (k-matched +0.006, corr(k,Q)=-0.001); mechanism = 2.58x biased removal of hub-mediated boundary edges; diffuse, no separate basin — compute claims in expectation only. Compute control now two-dimensional (exp_R): iteration-matched plain Leiden stays -0.0043..-0.0048 below seeded (p<0.03), convergence at 4.4x cost still short (directional); variance ratio 2.2x at all depths | exp_C, exp_E, exp_H sweeps; exp_N + exp_R SUMMARYs | Done, in paper; anatomy paragraph pending tex |
...
| C16 | L-Spar (Satuluri 2011, the founding sparsify-then-detect method): quality claim fails 14/14 cells under honest scoring (dQ_vs_matched -0.014..-0.404; naive scoring flips the sign in every cell = Artifact I); BUT its Jaccard signal is genuinely structure-aware — the only sparsifier whose selection signal the config null does NOT reproduce (deltaJ collapses to ~0/negative on rewirings). dQ_fixed retired as evidence (null reproduces it fully). Recovery: granularity control kills 5/6 cells; com-Amazon@0.542 exception (+0.062 vs over-matched control) = HYPOTHESIS, adjudication in exp_V. No speedup (0.87-1.35x) | exp_L SUMMARY (results/null_arm/recovery.csv) | Verified; NOT yet in paper — new section needed |
| C17 | (reserved, exp_V in flight) Can the verified structure-aware signal be deployed for an honest gain — weighting w=1+J with J-shuffle null, seeding, protected deletion | exp_V DESIGN.md pre-registered | Running |
