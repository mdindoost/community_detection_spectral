#!/usr/bin/env python3
"""
Experiment: DSpar Weighted vs Unweighted Comparison

Purpose: Compare two DSpar variants on large real-world graphs:
  - dspar: sparsify with paper method, drop weights, run Leiden unweighted
  - dspar_weighted: sparsify with paper method, keep weights, pass weights to Leiden
...
Usage:
    python exp_dspar_weighted_test.py
    python exp_dspar_weighted_test.py --datasets com-DBLP
    python exp_dspar_weighted_test.py --datasets com-DBLP,com-Amazon --dry_run
    python exp_dspar_weighted_test.py --max_edges 50000

Outputs (in results/exp_dspar_weighted_test/):
    - scalability_raw.csv
...
OUTPUT_DIR = Path(__file__).parent / "results" / "exp_dspar_weighted_test"
...
def sparsify_dspar_weighted(G: nx.Graph, alpha: float, seed: int) -> nx.Graph:
    """DSpar paper method -- returns weighted nx.Graph (weights preserved)."""
    return dspar_sparsify(G, retention=alpha, method='paper', seed=seed)
...
def sparsify(G: nx.Graph, method: str, alpha: float, seed: int) -> nx.Graph:
    ...
    elif method == 'dspar_weighted':
        return sparsify_dspar_weighted(G, alpha, seed)
    else:
...
def nx_to_igraph(G: nx.Graph) -> ig.Graph:
    """Convert NetworkX graph to igraph, preserving edge weights if present."""
    ...
    if G.number_of_edges() > 0:
        ...
        if 'weight' in G.edges[first_edge]:
...
def run_leiden(G: nx.Graph):
    ...
    Passes edge weights to Leiden if present.
...
def compute_modularity_fixed(G: nx.Graph, membership: dict) -> float:
    ...
    Uses igraph's modularity function, passing weights if present.
    """
    node_list = list(G.nodes())
    ig_graph = nx_to_igraph(G)
    weights = ig_graph.es['weight'] if 'weight' in ig_graph.es.attributes() else None
    ...
    return ig_graph.modularity(membership_list, weights=weights)
...
def generate_latex_table(df: pd.DataFrame, output_dir: Path, alpha: float = 0.8) -> Path:
    ...
    lines = [
        ...
        r"\centering",
        r"\small",
        f"\\caption{{DSpar weighted vs unweighted at $\\alpha = {alpha}$.}}",
        r"\label{tab:exp_dspar_weighted}",
...
def main():
    parser = argparse.ArgumentParser(
        description="Experiment: DSpar Weighted vs Unweighted Comparison"
    )
    parser.add_argument(
        "--datasets",
        type=str,
        default=None,
        help="Comma-separated list of datasets (default: com-DBLP,com-Amazon)"
    )
    parser.add_argument(
        "--max_edges",
        type=int,
    ...
    print("=" * 100)
    print("EXPERIMENT: DSPAR WEIGHTED VS UNWEIGHTED COMPARISON")
    print("=" * 100)
